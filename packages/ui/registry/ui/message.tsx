...
export type { Step } from "./timeline"
...
const ASSISTANT_BG = "#2a2a4a"
const USER_BG = "#1e3a2a"
...
function MessageText({ children, isLast = false }: {
  ...
  const { isStreaming, streamingCursor, backgroundColor } = useMessage()

  return (
    <text wrapMode="word">
      <span style={textStyle({ bg: backgroundColor })}>{children}</span>
      {isLast && isStreaming && (
        <span style={textStyle({ dim: true, bg: backgroundColor })}>{streamingCursor}</span>
      )}
    </text>
  )
}
...
function MessageToolInvocation({ part, toolColors }: {
  ...
  const { backgroundColor } = useMessage()
  ...
  return (
    <box flexDirection="column">
      <text>
        ...
        <span style={textStyle({ bg: backgroundColor })}>{" "}</span>
        <span style={textStyle({ fg: stateColor, bold: isActive, bg: backgroundColor })}>{toolName}</span>
        {isActive && <span style={textStyle({ dim: true, bg: backgroundColor })}>{" ..."}</span>}
      </text>
      {state === "result" && result !== undefined && (
        <text>
          <span style={textStyle({ dim: true, bg: backgroundColor })}>{"  \u2514\u2500 "}</span>
          <span style={textStyle({ dim: true, bg: backgroundColor })}>{String(result).slice(0, 120)}</span>
        </text>
      )}
    </box>
  )
...
function MessageFooter({ model, timestamp }: {
  ...
  return (
    <text>
      {model && <span style={textStyle({ dim: true })}>{model}</span>}
      {model && timestamp && <span style={textStyle({ dim: true })}>{" \u00B7 "}</span>}
      {timestamp && <span style={textStyle({ dim: true })}>{timestamp}</span>}
    </text>
  )
}
...
export function Message({
  ...
  const isUser = role === "user"
  const bg = backgroundColor ?? (isUser ? USER_BG : ASSISTANT_BG)

  return (
    <MessageContext.Provider value={{ role, isStreaming, streamingCursor, backgroundColor: bg }}>
      <box
        flexDirection="column"
        flexShrink={0}
