...
```bash title="Terminal"
bunx @gridland/container chalk-animation
```
...
|--------|---------|-------------|
| npm package | `chalk-animation` | Installs and runs the package bin |
...
Run any npm package that exposes a CLI binary:
...
bunx @gridland/container some-tui --no-network

# Set a 1GB memory limit
bunx @gridland/container heavy-app --memory 1g

# Forward arguments after --
bunx @gridland/container cjroth/gridland -- gradient --speed fast
